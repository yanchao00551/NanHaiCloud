...
import com.nanhai.securityoauth.api.UserController;
...
public class SysUserServiceImpl extends ServiceImpl<SysUserDao, SysUserEntity> implements SysUserService {
	...
	@Override
	public boolean verifyLoginSmscode(String mobile, String smscode,String requestHash,String tamp) {
		SimpleDateFormat sf = new SimpleDateFormat("yyyyMMddHHmmss");
		Date date = new Date(); // 获取当前时间
		String currentTime = sf.format(date);
		String hash = Md5Utils.hash(UserController.KEY + "@" + tamp + "@" + smscode);
		if (tamp.compareTo(currentTime) > 0) {
			if (hash.equalsIgnoreCase(requestHash)) {
				// 校验成功
				Map<String, Object> map = new HashMap<>();
				map.put("username", mobile);
				List<SysUserEntity> isUser = baseMapper.selectByMap(map);
				if(isUser.size() == 1){
					return true;
				}
			}
